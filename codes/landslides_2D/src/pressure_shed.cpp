...
{
bool mass_shed_flag = true; //change
int count =0;
...
//				(w[i0].v)/(w[i0].u*w[i0].u+w[i0].v*w[i0].v)*max((w[i1].b-w[i0].b),(w[i0].b-w[i_1].b))/dy));
	...
	//	Ang_Shed=Ang_Shed-2*PI/5*(omega*(pow(1+Gamma*w[j].b+epsilon*min_h,5)-pow(1+Gamma*w[j].b,5)))*dx;
		w[i0]=CV(  min_h,0,0,w[i0].b,w[i0].g,w[i0].x ,w[i0].y,true );
		//if (mass_shed_flag)
		//{
		//cout<<"mass shedding is happening"<<"  "<<i<<"   "<<j<<endl;
		//mass_shed_flag =  false;
		//}
		count++;
		} 
		w[i0].mass_shed =false;
		
	}}
	//cout<<"mass shedding is happening at "<<"  "<<count<<"   "<<"grid points"<<endl;
