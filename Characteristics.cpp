...
{   
    ...
    {	
		...
		wr[i]=CV(  wr[i].h,wr[i].u,wr[i].v,wr[i].b,wr[i].grad_b,wr[i].g,wr[i].x,om);
		//wr[i].Modify(wr[i].p,wr[i].q,wr[i].r,om);
        Reconstruct(wl[i],w[i-1],w[i],w[i+1],-1);            
        wl[i]=CV(  wl[i].h,wl[i].u,wl[i].v,wl[i].b,wl[i].grad_b,wl[i].g,wl[i].x,om);
  ...
  void Reconstruct(CV& w, CV w1, CV w2, CV w3, int sign )
  {
	...
	w.grad_b=w2.grad_b+sign*dx*Derivative(w1.grad_b,w2.grad_b,w3.grad_b)/2;
