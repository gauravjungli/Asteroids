...
def  energy(n,m,theta,t,roots,R=500/2,f=1.0,Q=2000,E=1e+6,k_s=1e+5):
    k_d=np.pi*2*f/Q
    G = 3/2 
    ...
    for i in range(0,n+1):
        for j in range(m):
            ...
            G+=B*legendre[i]*np.exp(-roots[i,j]**2*k_s*t)
...
if __name__=="__main__":
    ...
    t=np.exp(np.linspace(-7,7,1000))
    
   # filename=Output_File(parameters,"input",["roots.csv"])
    roots = np.loadtxt("roots.csv", delimiter=",")/R   
    ...
    theta = np.cos(np.linspace(0, np.pi,10)) 
    G = energy(n=n,m=m,R=R,theta=np.cos(np.pi),t=t,roots=roots)
    #print(G)
    plt.plot(t,G,label='Spherical')
    plt.xscale('log')  # Set x-axis to logarithmic scale
    plt.xlabel('Time (t)')
    plt.ylabel(r'$\epsilon_s(t)$')
    plt.title(r'$\epsilon_s(t)$ vs Time (log scale)')
    plt.grid(True)
    plt.legend()
    plt.show()
