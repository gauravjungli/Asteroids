...
{	
	...
	this->g=g;this->x=x;this->psi=Psi(*this,om); this->w=h+b;
	lambda= (b+h/2.0);
...
FS Source( CV w, CV w1, CV w2, CV w3, CV w4,  double om, double alpha)
{
	...
	double grad_b= epsilon*((w4.b+w3.b-w2.b-w1.b)/(2*dx))*((w1.h+w2.h+w3.h+w4.h)/4)*((w1.psi+w2.psi+w3.psi+w4.psi)/4)*(sin(w4.x)+sin(w3.x)+sin(w2.x)+sin(w1.x))/4;
	double pressure=(((pow((w1.v+w2.v)/2,2)+pow((w3.v+w4.v)/2,2))/2)*(1+2*epsilon*(w4.lambda+w3.lambda+w2.lambda+w1.lambda)/4)*((w1.h+w2.h+w3.h+w4.h)/4)+epsilon*((w1.psi+w2.psi+w3.psi+w4.psi)/4)*(pow((w1.h+w2.h)/2,2)+pow((w3.h+w4.h)/2,2))/4)*(sin(w4.x)+sin(w3.x)-sin(w2.x)-sin(w1.x))/(2*dx);

	source.q = pressure+(bf+fr)*w.h*sin(w.x)-grad_b;

	bf=2*om*w.u*(1+3*epsilon*w.lambda)*(cos(w.x)+epsilon*((w4.b+w3.b-w2.b-w1.b)/(2*dx))*sin(w.x))+alpha*sin(w.x)*(1+4*epsilon*w.lambda)-w.g.X3;
	if (pow(pow(w.u,2)+pow(w.v,2),0.5)>1e-10) fr=mu*(w.psi)*w.v/pow(pow(w.u,2)+pow(w.v,2),0.5)*(1+3*epsilon*w.b);
	else fr=(bf>0?-1:1)*min(abs(bf),mu*w.psi*(1+3*epsilon*w.b));
	source.r=-(bf+fr)*w.h*sin(w.x)*sin(w.x);
	return source;
}
